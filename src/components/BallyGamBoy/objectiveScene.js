...
class ObjectiveScene extends Phaser.Scene {
    ...
    dismissObjective() {
        if (this.objectiveDismissed) return; // Prevent re-entry
        this.objectiveDismissed = true;
    ...
    update() {
        ...
        if (this.currentStep >= 3 && !this.objectiveDismissed) {
