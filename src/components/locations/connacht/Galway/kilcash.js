...
// Function to enable movement controls
 enableMovementControls() {
    ...
    preload() {
        ...
        this.load.image('kilkashMap', '/phaser-resources/images/map1.png');
 ...
 this.movementControlsEnabled = true;
   ...
   if (dialogues) {
       ...
       this.textGa = this.add.text(350, 80, firstGaText, { fill: '#ffffff',fontFamily: 'aonchlo' });

       this.textEn = this.add.text(250, 278, firstEnText, { color: '#ffffff', fontFamily: 'anaphora'});
    ...
    // Create the kilkashMap element
    this.kilkashMap = this.add.sprite(0, 0, 'kilkashMap').setOrigin(0);

    const kilkashMapScale = Math.max(this.sys.game.config.width, this.sys.game.config.height) / Math.max(this.kilkashMap.width, this.kilkashMap.height) * 4;
    this.kilkashMap.setScale(kilkashMapScale);
    this.kilkashMap.x = (this.sys.game.config.width - this.kilkashMap.displayWidth) / 2;
    this.kilkashMap.y = (this.sys.game.config.height - this.kilkashMap.displayHeight) / 2;

// Ensure the bgOverlay moves with the background
this.kilkashMap.on('changedata-x', () => {
});
this.kilkashMap.on('changedata-y', () => {
});
...
update() {
    // Continuously update the position of bgOverlay to match kilkashMap
    // if (this.bgOverlay && this.kilkashMap) {
        // this.bgOverlay.x = this.kilkashMap.x;
        // this.bgOverlay.y = this.kilkashMap.y;
    // }
}
...
moveElement(direction) {
   ...
   const mapWidth = this.kilkashMap.width;
   const mapHeight = this.kilkashMap.height;
   ...
   this.tweens.add({
       targets: this.kilkashMap,
       x: targetX,
        y: targetY, // Negative y to move the map in the opposite direction
        duration: speed,
        ease: 'Linear',
        onComplete: () => {
            console.log(`Background Map Origin: (${Math.floor(this.kilkashMap.x)}, ${Math.floor(this.kilkashMap.y)})`);
        }
    });
}
